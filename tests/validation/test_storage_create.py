...
A storage-create registers an expr list in the burn account's data trie under
its head hash, creating a StorageRecord. The recipient must be the
BURN_ADDRESS and amount must be 0 (amount > 0 to burn is rejected).
"""
...
class TestStorageCreate(unittest.TestCase):
    ...
    def test_create_registers_storage_record(self):
        ...
        record_head = burn.data.get(self.node, list_id)
        ...
        self.assertGreater(burn.balance, burn_before)  # storage fee to burn
    ...
    def test_recipient_not_burn_creates_no_record(self):
        ...
        burn = self.block.accounts.get_account(BURN_ADDRESS, self.node)
        # recipient != BURN → no record created under burn data
        self.assertIsNone(burn.data.get(self.node, list_id))
